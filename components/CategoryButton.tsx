...
import { PRODUCT_CATEGORIES, getProductCategoryPath } from "@/data/products";
...
export default function CategoryButton({ currentCategoryId, children }: CategoryButtonProps) {
  const [isOpen, setIsOpen] = useState(false);
  const t = useTranslations('products.sidebar');
  const tCategories = useTranslations('products');
        ...
        <div className="h-full overflow-y-auto pt-20 pb-6 px-4">
          ...
          <ul className="space-y-1">
            {PRODUCT_CATEGORIES.map((category) => (
              <li key={category.id}>
                <Link 
                  href={getProductCategoryPath(category.id)}
                  onClick={() => setIsOpen(false)}
                ...
                >
                  {tCategories(`categories.${category.id}.name`)}
                </Link>
              </li>
            ))}
          </ul>
